...
def main():
    ...
    calibrated = leaderboard[leaderboard["calibrated"]]
    ...
    report.append(
        f"Models that successfully utilized calibration: {', '.join(calibrated['model'].tolist()) if len(calibrated) > 0 else 'None (Uncalibrated outperformed)'}"
    )
    ...
    report.append(
        f"Successfully generated states for {len(states)} historical teams without data leakage."
    )
    ...
    report.append(
        f"**Consistency Check**: `{'PASSED' if is_consistent else 'FAILED'}` (Deterministic outputs for identical states)"
    )
    ...
    report.append(
        f"- **Numerical Stability Check**: `{'PASSED' if abs(sum_probs - 1.0) < 1e-5 else 'FAILED'}`"
    )
