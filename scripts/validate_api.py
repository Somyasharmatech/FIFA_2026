...
def main():
    ...
    # POST /predict
    res = client.post("/predict", json={"home_team": "Argentina", "away_team": "France"})
    assert res.status_code == 200
    ...
    report.append(f"- `POST /predict` (Unknown Team): Rejected correctly with {res.status_code} ({res.json()['detail']})")
    
    # Missing parameters
    res = client.post("/predict", json={"home_team": "France"})
    assert res.status_code == 422
    report.append(f"- `POST /predict` (Missing param): Rejected correctly with {res.status_code} (Unprocessable Entity)")
    
    # Unknown team GET
    res = client.get("/team/Atlantis")
    assert res.status_code == 404
    report.append(f"- `GET /team/{{name}}` (Unknown Team): Rejected correctly with {res.status_code}")
    ...
    report.append(f"- **Estimated Throughput**: {throughput:.2f} req/sec (single thread)")
    ...
    report.append("- Pydantic request and response schemas are strictly enforced and documented.")
    
    report.append("\n## 5. Deployment Readiness Assessment")
    report.append("The API is fully responsive, robust against malformed inputs, appropriately raises HTTP error codes (400, 404, 422, 500) rather than failing silently, and is extremely performant under sequential load. Cleared for Dockerization.")
