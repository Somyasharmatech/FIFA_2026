...
def get_team_analysis(team_name: str) -> TeamProfile:
    ...
    if not engine or team_name not in engine._states:
        raise HTTPException(
            status_code=404, detail="Team not found or pipeline not run."
        )
...
def predict_match(req: MatchRequest) -> MatchResponse:
    ...
    xg_home = max(
        0.0,
        1.2
        + (hs.attack_strength - as_.defense_strength) * 1.2
        + (hs.elo - as_.elo) / 500,
    )
    xg_away = max(
        0.0,
        1.2
        + (as_.attack_strength - hs.defense_strength) * 1.2
        + (as_.elo - hs.elo) / 500,
    )
...
def simulate_tournament() -> Dict[str, Any]:
    ...
    top_10 = sims.head(10)[["team", "champion_prob", "final_prob"]].to_dict(
        orient="records"
    )
