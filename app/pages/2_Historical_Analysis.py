...
hero(
    "Historical Analysis",
    "Goal trends, dominant nations, and World Cup history "
...
left.plotly_chart(
    charts.line_chart(
        goals_per_year(cleaned), "year", "avg_goals", "Average goals per match by year"
    ),
    width="stretch",
)
right.plotly_chart(
    charts.bar_chart(
        average_goals_by_decade(cleaned),
        "decade",
        "avg_goals",
...
right.plotly_chart(
    charts.line_chart(
        wc_by_year, "year", "avg_goals", "World Cup goals per match by edition"
    ),
    width="stretch",
)
