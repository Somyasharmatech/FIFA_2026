...
class PDFReport(FPDF):
    def header(self):
        ...
        self.cell(
            0,
            10,
            f"{config.tournament.name} {config.tournament.year} - AI Analytics Report",
            border=False,
            align="C",
        )
        self.ln(15)
...
if st.button("Generate Professional PDF Report", type="primary"):
    with st.spinner("Compiling Analytics & Building PDF..."):
        ...
        pdf.multi_cell(
            0,
            8,
            f"This report outlines the AI-driven forecast for the {config.tournament.name} {config.tournament.year}. Based on 100,000 Monte Carlo simulations utilizing a {metadata['model_name']} model (Accuracy: {metadata['leaderboard'][0]['accuracy']:.1%}), we have evaluated the probabilities of all participating nations.",
        )
        pdf.ln(10)
        ...
        pdf.multi_cell(
            0,
            8,
            f"The predictive engine is powered by {metadata['model_name']}, selected over 5 other algorithms. It was evaluated using Time-Series Cross Validation.",
        )
        pdf.ln(5)
        ...
        for metric, val in metadata["leaderboard"][0].items():
            if isinstance(val, float):
                pdf.cell(
                    0, 8, f"- {metric.replace('_', ' ').title()}: {val:.3f}", ln=True
                )
        ...
        st.download_button(
            ...
            data=pdf_bytes,
            file_name="FIFA_2026_Analytics_Report.pdf",
