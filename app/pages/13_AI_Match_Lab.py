...
hero("AI Match Lab", "Tweak form, team strength, and momentum to instantly simulate hypothetical matchups. See exactly how specific attributes shift the balance of power.")
...
home = col1.selectbox("Team 1 (Home)", teams, index=teams.index("Argentina") if "Argentina" in teams else 0)
away = col2.selectbox("Team 2 (Away)", teams, index=teams.index("France") if "France" in teams else 1)
...
home_form = c3.slider(f"{home} Form (Win Rate)", 0.0, 1.0, float(home_state.form_win_rate))
away_form = c4.slider(f"{away} Form (Win Rate)", 0.0, 1.0, float(away_state.form_win_rate))
...
home_defense = c7.slider(f"{home} Defense", 0.0, 3.0, float(home_state.defense_strength))
away_defense = c8.slider(f"{away} Defense", 0.0, 3.0, float(away_state.defense_strength))
...
if st.button("Simulate Match", type="primary"):
    with st.spinner("Calculating alternative reality..."):
        ...
        xg_home = max(0.0, 1.2 + (home_attack - away_defense) * 1.2 + (home_elo - away_elo)/500)
        xg_away = max(0.0, 1.2 + (away_attack - home_defense) * 1.2 + (away_elo - home_elo)/500)
        ...
        st.info(f"**Confidence Score:** {confidence:.2f}x (Higher means the AI is more certain of the favorite)")
