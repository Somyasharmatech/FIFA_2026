...
hero(
    "Team DNA",
    "An advanced breakdown of a nation's tactical profile, knockout experience, and historical consistency.",
)
...
selected_team = st.selectbox(
    "Select Nation", teams, index=teams.index("Brazil") if "Brazil" in teams else 0
)
...
team_matches = cleaned[
    (cleaned["home_team"] == selected_team) | (cleaned["away_team"] == selected_team)
]
...
for year, group in team_matches.groupby("year"):
    wins = sum(
        (group["home_team"] == selected_team) & (group["outcome"] == "home_win")
...
if shootouts is not None:
    team_shootouts = shootouts[
        (shootouts["home_team"] == selected_team)
        | (shootouts["away_team"] == selected_team)
    ]
    if len(team_shootouts) > 0:
        wins = len(team_shootouts[team_shootouts["winner"] == selected_team])
        pressure_rating = (
            f"{wins / len(team_shootouts):.1%} win rate ({wins}/{len(team_shootouts)})"
        )
...
c2.markdown(
    f'<div class="glass-card"><div class="metric-label">Passing Profile</div><div class="metric-value" style="font-size:1.2rem;">{passing_profile}</div></div>',
    unsafe_allow_html=True,
)
c3.markdown(
    f'<div class="glass-card"><div class="metric-label">Knockout Experience</div><div class="metric-value" style="font-size:1.2rem;">{knockout_experience} WC matches</div></div>',
    unsafe_allow_html=True,
)
c4.markdown(
    f'<div class="glass-card"><div class="metric-label">Historical Consistency</div><div class="metric-value" style="font-size:1.2rem;">{consistency_score:.2f} / 1.0</div></div>',
    unsafe_allow_html=True,
)
...
c5.markdown(
    f'<div class="glass-card"><div class="metric-label">Goal Threat</div><div class="metric-value" style="font-size:1.2rem;">{goal_threat:.2f} Index</div></div>',
    unsafe_allow_html=True,
)
c6.markdown(
    f'<div class="glass-card"><div class="metric-label">Defensive Stability</div><div class="metric-value" style="font-size:1.2rem;">{defensive_stability:.2f} Index</div></div>',
    unsafe_allow_html=True,
)
c7.markdown(
    f'<div class="glass-card"><div class="metric-label">Pressure Rating</div><div class="metric-value" style="font-size:1.2rem;">{pressure_rating}</div></div>',
    unsafe_allow_html=True,
)
c8.markdown(
    f'<div class="glass-card"><div class="metric-label">Set Piece Efficiency</div><div class="metric-value" style="font-size:1.2rem;">{set_piece_eff}</div></div>',
    unsafe_allow_html=True,
)
...
goals_timeline = []
for year, group in team_matches.groupby("year"):
    gf = sum(group[group["home_team"] == selected_team]["home_score"]) + sum(
        group[group["away_team"] == selected_team]["away_score"]
    )
    ga = sum(group[group["home_team"] == selected_team]["away_score"]) + sum(
        group[group["away_team"] == selected_team]["home_score"]
    )
...
if not df_goals.empty:
    st.plotly_chart(
        charts.line_chart(df_goals, "year", "Goals For", "Goals Scored Over Time"),
        width="stretch",
    )
...
series = {
    selected_team: [
        state.elo / 2500,
        state.form_win_rate,
        state.attack_strength / 3.0,
        state.defense_strength / 3.0,
    ],
    "Global Average": [avg_elo / 2500, avg_form, avg_attack / 3.0, avg_defense / 3.0],
}
st.plotly_chart(
    charts.radar_compare(categories, series, "Team Attribute Radar"), width="stretch"
)
